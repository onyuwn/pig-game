...
void main() {
    ...
    if(shellIndex <= shellCount / 2.0) {
        ...
        FragColor = vec4(leafColor.xyz, 1.0);
    } else {
        ...
        float rand = mix(noiseMin, noiseMax, hash( floor( TexCoords * density * ( (.01 * cos(fragPos.x) + 1.0) + (.01 * sin(fragPos.z) + 1.0) ) ) ));
        ...
        // );
        FragColor = vec4(
            vec3(((1.5 - (pow(shellHeight - .25, shellAttenuation))) * leafColor.xyz)) * (ambient + diff),
            leafColor.a
        );
    }
}